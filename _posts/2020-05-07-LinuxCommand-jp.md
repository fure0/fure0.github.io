...
#### ファイル/ディレクトリ検索コマンド
* find / -name wonderit
* find 検索開始位置 名前で検索 検索ワード

#### grep
* 必要な情報を得る時に使用

#### コマンド結果で私が必要な部分だけを探す時
* ps -ef | grep test
* ps -ef の結果でtestが含まれてるのだけ見せる

* grep test a.c
* a.c ファイルでtestがある部分を見せる
...
#### cp
* コピー
* cp [コピーするファイル名] [コピーされるファイル名]
* -r 下位リストまでコピー

#### mv
* ファイル移動、ファイル名変更
* mv a.c a.e とするとファイル名が a.c が a.e に変更
* mv a.c [ディレクトリ名] すれば当該ディレクトリに移動
...
#### mkdir
* ディレクトリ作り
* -p 記述された経路がなければ自動的に作る) mkdir test1/test2/test3

#### rmdir
* ディレクトリを削除する

#### touch
* 空きファイル作成ex) touch test
